...
class SchedulesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->merge(['create_by' => $currentUserId]); // merge created by filed in request array

        $recordExits = schedules::recordExits($request); // check data already book or not

        if(isset($recordExits) && !empty($recordExits)){
            return redirect()->back()->with('error', 'This slot already book please try another with time');
        }
